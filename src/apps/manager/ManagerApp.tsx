      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <div className="bg-white/85 dark:bg-[#0D1524]/85 backdrop-blur-xl border-b border-slate-200/70 dark:border-slate-800/70 px-5 sm:px-8 py-4 transition-colors duration-300 z-10">
          <div className="flex items-center justify-between gap-2 sm:gap-4">
            <div className="flex-1 min-w-0 relative pr-1 sm:pr-2">
              <button 
                onClick={() => setIsBuildingSelectorOpen(!isBuildingSelectorOpen)}
                className="w-full text-left text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-1 sm:gap-2 hover:opacity-80 transition-opacity"
              >
                <span className="truncate block">{selectedBuilding.name}</span>
                <ChevronDown className={`w-4 h-4 sm:w-5 sm:h-5 shrink-0 text-slate-400 transition-transform ${isBuildingSelectorOpen ? 'rotate-180' : ''}`} />
              </button>
              <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-0.5 flex items-center gap-1 sm:gap-1.5 font-medium">
