...
console.log(`[1/4] Building web bundle with VITE_APP_TARGET=${target}...`);
...
console.log(`[2/4] Applying ${configFile} -> capacitor.config.ts...`);
fs.copyFileSync(path.join(rootDir, configFile), path.join(rootDir, 'capacitor.config.ts'));

// 3. Update android/app/build.gradle with the target's unique applicationId
console.log(`[3/4] Updating android/app/build.gradle applicationId -> ${appId}...`);
...
console.log(`[3/4b] Updating android strings.xml app_name -> ${appName}...`);
...
console.log(`[4/4] Running npx cap sync android...`);
